...
class Api::CreatePostsControllerTest < ActionController::TestCase
  
  test "should route to create_posts#mail" do
    assert_routing(
        {:method => 'post', :path => "/api/create_posts.json"}, 
        {:format => "json", :controller => "api/create_posts", :action => "mail" })
  end
