class Api::ApiController < ActionController::Base
  ...
  rescue_from Facebook::InvalidAccessTokenError do |e|
    notify_airbrake(e)
    Rails.logger.error "Api::ApiController 406 (InvalidAccessTokenError) rescue: #{e.message}"
  ...
  rescue_from NoMethodError do |e|
    notify_airbrake(e)
    Rails.logger.error "Api::ApiController 405 (NoMethodError) rescue: #{e.message}"
  ...
  rescue_from ActiveRecord::RecordNotFound do |e|
    notify_airbrake(e)
    Rails.logger.error "Api::ApiController 404 (ActiveRecord::RecordNotFound) rescue: #{e.message}"
  ...
  rescue_from ActiveRecord::RecordInvalid do |e|
    notify_airbrake(e)
    Rails.logger.error "Api::ApiController 400 (ActiveRecord::RecordInvalid) rescue: #{e.message}"
